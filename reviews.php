...
if (isset($_GET['page_id'])) {
    ...
    $stmt = $pdo->prepare('INSERT INTO webreviews (page_id, name, content, rating, submit_date) VALUES (?,?,?,?,NOW())');
    ...
    $stmt = $pdo->prepare('SELECT * FROM webreviews WHERE page_id = ? ORDER BY submit_date DESC');
    $stmt->execute([$_GET['page_id']]);
    $reviews = $stmt->fetchAll(PDO::FETCH_ASSOC);
    // Get the overall rating and total amount of reviews
    $stmt = $pdo->prepare('SELECT AVG(rating) AS overall_rating, COUNT(*) AS total_reviews FROM webreviews WHERE page_id = ?');
    $stmt->execute([$_GET['page_id']]);
